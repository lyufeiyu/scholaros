...
flowchart TB
    ...
    WF --> ART["JSON / Markdown / DOCX / TeX / ZIP 制品"]
...
6. **制品优先：** 每阶段留可检查的 JSON/Markdown；交付阶段还可派生 DOCX、TeX、哈希清单和不含原始上传材料的 ZIP。
...
0.2 版新增八区项目工作台、六类任务、可配置的学习与交付目标、贡献/图件选择、意见返修、DOCX/TeX 导出和可核验的本地交付包。页级全文解析、论断级证据、实验执行、最终媒体渲染、协作与专家审批仍属于后续工作。
