...
- 网络按钮把 API 错误显示为页面提示，项目状态由轮询更新。页面区分中断运行、断点继续、阶段确认和从头运行。每个阶段完成会自动保存版本，工作流完成再保存最终版；项目页用版本时间线支持在“当前研究项目”界面查看任一历史版本，历史制品从对应 revision 读取。局部重做仍保留为 CLI/API 能力，详见[分步研究与恢复](workflow.md)。
