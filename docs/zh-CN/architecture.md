...
stateDiagram-v2
    ...
    Reviewing --> Revising
    Revising --> Completed
    Reviewing --> NeedsAttention: 高优先级问题
    NeedsAttention --> Scoping: 人工修改后 restart
```

每个阶段先保存当前状态再执行，完成后保存制品和事件。进程崩溃时可以从当前阶段重跑；单一论文源失败只记录，不使整条工作流失败。
...
11. **重跑制品隔离：** restart 保留用户 source 资料并移除上一轮生成制品，防止新一轮失败后把旧稿误当当前结果。
